...
public class Mapper {
    ...
    public List<ItemDTO> toItemEntityToDTOs(List<Item> items) {
        Stream<ItemDTO> itemDTOStream = items.stream().map(item -> modelMapper.map(item, ItemDTO.class));
        return itemDTOStream.toList();
    }
}
